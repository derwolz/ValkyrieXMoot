...
function buildNavGrid({ bounds, buildingAABBs, roadSegments, intersections, interestPoints }) {
  ...
  const inflatedAABBs = buildingAABBs.map(a => ({
    minX: a.minX - mootR, maxX: a.maxX + mootR,
    minZ: a.minZ - mootR, maxZ: a.maxZ + mootR,
  }));
  ...
  function cellToWorld(col, row) { return { x: originX + col * cs, z: originZ + row * cs }; }
  ...
  return { cellSize: cs, cols, rows, origin: { x: originX, z: originZ },
           walkable, costMul, interestPoints, worldToCell, cellToWorld, idx };
}
...
class MinHeap {
  ...
  push(n) { this._data.push(n); this._bubbleUp(this._data.length - 1); }
  pop() { const t = this._data[0]; const l = this._data.pop(); if (this._data.length) { this._data[0] = l; this._siftDown(0); } return t; }
  ...
  _siftDown(i) { const n=this._data.length; while(true){let m=i,l=2*i+1,r=2*i+2; if(l<n&&this._data[l].f<this._data[m].f)m=l; if(r<n&&this._data[r].f<this._data[m].f)m=r; if(m===i)break; [this._data[m],this._data[i]]=[this._data[i],this._data[m]]; i=m;} }
}

function heuristic(c1,r1,c2,r2){const dx=Math.abs(c1-c2),dz=Math.abs(r1-r2);return(dx+dz)+(SQRT2-2)*Math.min(dx,dz);}
function idxToColRow(i,cols){return{col:i%cols,row:Math.floor(i/cols)};}
...
function smoothPath(path) {
  ...
  for (let i=1;i<path.length-1;i++){
    ...
    const cx=next.x-prev.x,cz=next.z-prev.z,ex=cur.x-prev.x,ez=cur.z-prev.z;
...
function reconstructPath(parent, goalIdx, cols, cellToWorld) {
  const indices = [];
  let cur = goalIdx;
  while (cur !== -1) { indices.push(cur); cur = parent[cur]; }
  indices.reverse();
  const path = indices.map(i => { const {col,row}=idxToColRow(i,cols); return cellToWorld(col,row); });
...
function findPath(grid, startWorld, goalWorld) {
  ...
  if (startIdx === goalIdx) { const s=idxToColRow(startIdx,cols); return [cellToWorld(s.col,s.row)]; }
  ...
  while (open.size > 0 && iter++ < NAV.aStarMaxCells) {
    ...
    for (let dr=-1; dr<=1; dr++) for (let dc=-1; dc<=1; dc++) {
      ...
      if (dr!==0&&dc!==0){const n1=idx(cur.col+dc,cur.row),n2=idx(cur.col,cur.row+dr); if((n1<0||!walkable[n1])||(n2<0||!walkable[n2]))continue;}
      const moveCost=(dr!==0&&dc!==0)?SQRT2:1.0;
      const ng=g[ci]+moveCost*costMul[ni];
      if (ng<g[ni]){g[ni]=ng;parent[ni]=ci;open.push({i:ni,f:ng+heuristic(nc,nr,gc_.col,gc_.row)});}
    }
  }
  return [];
}
...
function deepestPenetration(ox, oz, cx, cz, aabbs, r) {
  let bestDepth = 0, best = null;
  for (const aabb of aabbs) {
    const npx = Math.max(aabb.minX, Math.min(cx, aabb.maxX));
    const npz = Math.max(aabb.minZ, Math.min(cz, aabb.maxZ));
    const dx=cx-npx, dz=cz-npz, distSq=dx*dx+dz*dz;
    if (distSq >= r*r) continue;
    let nx,nz,depth;
    if (distSq < 1e-10) {
      const movX=cx-ox, movZ=cz-oz;
      if(Math.abs(movX)>=Math.abs(movZ)){nx=movX>=0?-1:1;nz=0;depth=Math.max(0,(nx<0?cx-aabb.minX:aabb.maxX-cx)+r);}
      else{nx=0;nz=movZ>=0?-1:1;depth=Math.max(0,(nz<0?cz-aabb.minZ:aabb.maxZ-cz)+r);}
    } else {
      const dist=Math.sqrt(distSq);nx=dx/dist;nz=dz/dist;depth=r-dist;
    }
    if(depth>bestDepth){bestDepth=depth;best={px:nx*depth,pz:nz*depth,nx,nz};}
  }
  return best;
}

function moveWithCollision(pos, vx, vz, dt, aabbs, radius) {
  let nx=pos.x+vx*dt, nz=pos.z+vz*dt;
  for (let pass=0; pass<2; pass++) {
    const pen=deepestPenetration(pos.x, pos.z, nx, nz, aabbs, radius);
    if(!pen) break;
    nx+=pen.px; nz+=pen.pz;
    if(pass===0){const dot=vx*pen.nx+vz*pen.nz;vx-=dot*pen.nx;vz-=dot*pen.nz;}
  }
  return {x:nx,z:nz};
}
...
console.log('\n[Test 1] buildNavGrid basics');
{
  ...
  const grid = buildNavGrid({
    bounds, buildingAABBs: [], roadSegments: [...roadSegments, ...alleySegs],
    intersections, interestPoints: [], alleys: alleySegs,
  });
  ...
  const grid2 = buildNavGrid({
    bounds, buildingAABBs: [], roadSegments: [...roadSegments, alley],
    intersections, interestPoints: [], alleys: [alley],
  });
  ...
  const grid3 = buildNavGrid({
    bounds, buildingAABBs: [{ minX:-1, maxX:1, minZ:-1, maxZ:1 }],
    roadSegments, intersections, interestPoints: [], alleys: [],
  });
  ...
  assert('worldToCell/cellToWorld round-trips within 1 m',
...
console.log('\n[Test 2] findPath');
{
  ...
  const roadSegments = [
    { a:{x:0,z:-50}, b:{x:0,z:50}, width:10, kind:'local' },
  ];
  const grid = buildNavGrid({ bounds, buildingAABBs: [], roadSegments,
                               intersections: [], interestPoints: [], alleys: [] });
  ...
  assert('path ends near goal',                  Math.abs(path[path.length-1].x) < 2 && Math.abs(path[path.length-1].z - 30) < 2);
  ...
  const gridBlocked = buildNavGrid({ bounds: bounds2, buildingAABBs: wallAABB,
                                    roadSegments: segs2, intersections: [],
  ...
  const blocked = pathBlocked.length === 0 ||
    Math.abs(pathBlocked[pathBlocked.length-1].z - 30) > 5;
