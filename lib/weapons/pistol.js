...
//   pistol.aimAt()       — drives the parallax offset via mouse NDC
...
function _resizeLaserCanvas() {
  ...
  if (_laserCanvas.width === width && _laserCanvas.height === height) return;
  _laserCanvas.width = width;
  _laserCanvas.height = height;
  _laserCanvasDirty = true;
}

// Active beam descriptors: { x0, y0, x1, y1, hx, hy, hasHit, life, maxLife }
// hx,hy = projected screen coords of 3D hit point (when hasHit is true)
const _beams = [];

function _renderBeams() {
  if (!_laserCtx || !_laserCanvas) return;
  if (_beams.length === 0) {
    if (_laserCanvasDirty) {
      _laserCtx.clearRect(0, 0, _laserCanvas.width, _laserCanvas.height);
      _laserCanvasDirty = false;
    }
    return;
  }
  _laserCtx.clearRect(0, 0, _laserCanvas.width, _laserCanvas.height);
  ...
  for (const b of _beams) {
    ...
    _laserCtx.stroke();
    _laserCtx.restore();
    // Impact spark lines radiating from 3D hit point
    if (b.hasHit) {
      const sparkCount = PISTOL.sparkCount;
      const sparkLength = PISTOL.sparkLength * t;
      _laserCtx.save();
      _laserCtx.globalAlpha = t * 0.9;
      _laserCtx.strokeStyle = PISTOL.sparkColor;
      _laserCtx.lineWidth = 1.5;
      _laserCtx.lineCap = 'round';
      _laserCtx.shadowBlur = 6;
      _laserCtx.shadowColor = PISTOL.sparkColor;
      for (let i = 0; i < sparkCount; i++) {
        const angle = (i / sparkCount) * Math.PI * 2;
        _laserCtx.beginPath();
        _laserCtx.moveTo(b.hx, b.hy);
        _laserCtx.lineTo(
          b.hx + Math.cos(angle) * sparkLength,
          b.hy + Math.sin(angle) * sparkLength,
        );
        _laserCtx.stroke();
      }
      _laserCtx.restore();
...
export class Pistol {
  ...
  // ── aimAt ──────────────────────────────────────────────────────────────────
  /**
   * Reads the shared mouse NDC state to compute a 2-D parallax shift.
   * The optional argument is accepted for legacy callers but ignored.
   *
  ...
  aimAt(_worldPt) {
    ...
    const transform = `translate(${this._ox}px,${this._oy}px) rotate(${-tilt}deg)`;
   ...
   * @param {THREE.Vector3} [hitWorldPt]  — 3D world point where the beam terminates.
   *   When provided the beam ends at the projected screen position of this point
   *   and an impact spark is drawn there instead of extending to the viewport edge.
   */
  tryFire(originWorld, camera, hitWorldPt) {
    ...
    // Beam end: use projected 3D hit point when available; otherwise extend
    // toward the cursor past the viewport edge (original behaviour).
    let x1;
    let y1;
    let hx;
    let hy;
    let hasHit;
    if (hitWorldPt && camera) {
      _projTmp.copy(hitWorldPt).project(camera);
      hx = (_projTmp.x * 0.5 + 0.5) * window.innerWidth;
      hy = (1 - (_projTmp.y * 0.5 + 0.5)) * window.innerHeight;
      x1 = hx;
      y1 = hy;
      hasHit = true;
    } else {
      const cx = window.__vxmMouse ? (window.__vxmMouse.ndcX * 0.5 + 0.5) * window.innerWidth : x0;
      const cy = window.__vxmMouse
        ? (1 - (window.__vxmMouse.ndcY * 0.5 + 0.5)) * window.innerHeight
        : y0;
      const dx = cx - x0;
      const dy = cy - y0;
