...
export class Pistol {
  ...
  tryFire() {
    ...
    // Spawn a laser beam from muzzle tip toward the cursor.
    const muzzle = this.getScreenPos();
    const cx = window.__vxmMouse ? (window.__vxmMouse.ndcX * 0.5 + 0.5) * window.innerWidth  : muzzle.x;
    const cy = window.__vxmMouse ? (1 - (window.__vxmMouse.ndcY * 0.5 + 0.5)) * window.innerHeight : muzzle.y;

    // Extend beam to viewport edge along the muzzle→cursor direction.
    const dx = cx - muzzle.x;
    const dy = cy - muzzle.y;
    const len = Math.sqrt(dx * dx + dy * dy) || 1;
    const scale = Math.max(window.innerWidth, window.innerHeight) * 2 / len;
    _beams.push({
      x0: muzzle.x,
      y0: muzzle.y,
      x1: muzzle.x + dx * scale,
      y1: muzzle.y + dy * scale,
