...
const REACTIONS = {
  ...
  turboCelebrating: { label: '🥳', image: '/data/reactions/turbo-celebrating.png' },
  shootCelebrating: { label: '🥳', image: '/data/reactions/shoot-celebrate.png' },
...
/**
 * @param {'neutral'|'happy'|'celebrating'|'turboCelebrating'|'shootCelebrating'|'smug'|'angry'|'pained'|'panicked'|'turbo'} state
 * @param {number} [revertAfterMs=0]
 */
export function setFace(state, revertAfterMs = 0) {
  if (!els) return;
  const next = REACTIONS[state] ? state : 'neutral';
  const isTimedReaction = revertAfterMs > 0;
  if (!isTimedReaction) {
    _baseState = next === 'neutral' ? 'neutral' : next;
  }
  // Skip redundant re-apply when the state and timer setup haven't changed.
  if (next === _currentState && !isTimedReaction && !revertTimer) return;
  ...
  if (isTimedReaction) {
    revertTimer = setTimeout(() => {
      const revertState = REACTIONS[_baseState] ? _baseState : 'neutral';
      _currentState = revertState;
      _applyReaction(REACTIONS[revertState]);
      revertTimer = null;
    }, revertAfterMs);
  }
}
