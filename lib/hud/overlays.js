...
export function updateHud(game) {
  ...
  // ── Score & Timer ──────────────────────────────────────────────────────
  if (els.scoreDisplay) {
    els.scoreDisplay.textContent = game.score != null ? String(game.score) : '0';
  }
  if (els.timerDisplay && game.timeRemaining != null) {
    const secs = Math.ceil(game.timeRemaining);
    els.timerDisplay.textContent = `${secs}s`;
    const warning = secs <= TARGET.timeWarningS;
    els.timerDisplay.classList.toggle('warning', warning);
  }
  if (els.comboDisplay && game.combo != null) {
    const show = game.combo > 1;
    els.comboDisplay.classList.toggle('show', show);
    if (show) els.comboDisplay.textContent = `×${game.combo} COMBO`;
  }
  ...
  els.fill.classList.toggle('boosting', !!game.boosting);
...
export function setGameOverVisible(visible, onRestart, opts) {
  ...
  if (opts && opts.title && els.gameoverTitle) els.gameoverTitle.textContent = opts.title;
  if (opts && opts.sub   && els.gameoverSub)   els.gameoverSub.textContent   = opts.sub;
...
export function updateTargetHud(game, camera, targetPos) {
  ...
  if (els.targetCountdown) {
    ...
    els.targetCountdown.classList.toggle('active', hasTarget);
    if (hasTarget) {
      const secs = Math.ceil(game.targetCountdown ?? 0);
      els.targetCountdown.textContent = `TARGET  ${secs}s`;
      els.targetCountdown.classList.toggle('urgent', secs <= (TARGET.targetWarningS ?? 5));
    }
  }
  ...
  if (els.targetArrow && els.targetDistance) {
    const hasTarget = playing && targetPos != null;
    els.targetArrow.classList.toggle('active', hasTarget);
    els.targetDistance.classList.toggle('active', hasTarget);
    if (hasTarget) {
      // Project target world position to NDC.
      const proj = targetPos.clone().project(camera);
      // Angle from screen center toward the projected point.
      const angle = Math.atan2(proj.x, -proj.y);   // atan2(dx, -dy) → CSS rotation angle
      ...
      els.targetDistance.textContent = `${dist}m`;
    }
  }
}
