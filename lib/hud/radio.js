...
async function _loadStationTracks(station) {
  try {
    const res = await fetch(`/data/music/${station.slug}/`, { headers: { 'Accept': 'application/json' } });
    if (!res.ok) return [];
...
export function createRadio() {
  ...
  function _saveChannel() {
    try { localStorage.setItem(STORAGE_KEY, String(channelIndex)); } catch {}
  }
  ...
  audio.addEventListener('loadedmetadata', () => {
    ...
    const station = _stationData();
    if (!station || !isFinite(audio.duration)) return;
  ...
  function _playCurrent() {
    ...
    if (!track.src) { audio.pause(); audio.removeAttribute('src'); return; }
    if (audio.src !== track.src) audio.src = track.src;
    audio.currentTime = 0;
    audio.play().then(() => { _gestureUnlocked = true; }).catch(() => {
  ...
  function _applyStyle(station) {
    ...
    const r = parseInt(station.accent.slice(1, 3), 16);
    const g = parseInt(station.accent.slice(3, 5), 16);
    const b = parseInt(station.accent.slice(5, 7), 16);
    dom.widget.style.setProperty('--radio-accent-glow', `rgba(${r},${g},${b},0.35)`);
  }

  function _renderTrack() {
    const station = _stationData();
    if (!station) return;
    if (station.tracks.length === 0) { _renderEmptyStation(); return; }
    const track = station.tracks[trackIndex];
    dom.station.textContent = station.name;
    dom.track.textContent   = track.title;
  ...
  function _renderEmptyStation() {
    ...
    if (!station) return;
    dom.station.textContent = station.name;
    dom.track.textContent   = '— no tracks —';
    dom.artist.textContent  = `drop files in /data/music/${station.slug}/`;
    _applyStyle(station);
    dom.progressFill.style.width = '0%';
  }
  ...
  return {
    ...
    cycleChannel() {
      ...
      if (station) {
        trackIndex = 0;
        trackElapsed = 0;
        dom.widget.classList.add('on');
        _renderTrack();
        _updateProgress();
        _playCurrent();
      } else {
        dom.widget.classList.remove('on');
        audio.pause();
        audio.removeAttribute('src');
      }
    },
    ...
    tick(dt) {
      ...
      if (track.src && !audio.paused && isFinite(audio.duration)) {
