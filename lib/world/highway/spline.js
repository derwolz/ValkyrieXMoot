...
function buildCircuitControlPoints({ rand, width, length, deckHeight }) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const organicBend = Math.sin(angle * 3 + phase) * 0.035 + Math.sin(angle * 2 - phase * 0.7) * 0.025;
...
export function buildHighwaySpline({
  ...
  const { points: controlPts, centerZ } = buildCircuitControlPoints({ rand, width, length, deckHeight });
  ...
  const rampReservationHalfWidth = HIGHWAY.rampReservationHalfWidth ?? Math.max(16, rampHalfWidth + 8);
