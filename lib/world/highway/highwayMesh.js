...
export function buildHighwayMesh({
  ...
  const deckHalfWidth = readHighwayConfigNumber('expresswayHalfWidth', readHighwayConfigNumber('roadHalfWidth', 18));
  const rampHalfWidth = readHighwayConfigNumber('rampHalfWidth', 7)
    + Math.max(0, readHighwayConfigNumber('rampShoulderWidth', 0));
  const deckHeight = readHighwayConfigNumber('deckHeight', readHighwayConfigNumber('overpassHeight', 18));
  ...
  function appendRibbon(points, halfWidth, fallbackRelativeY, kind, collectRampAABB) {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      if (lenSq > 0.0001) {
        ...
        const lateralTolerance = Math.max(0, readHighwayConfigNumber('surfaceQueryLateralTolerance', 0.15));
  ...
  if (zoneMap && Array.isArray(reservationPolylines) && reservationPolylines.length > 0
    ...
    for (const ramp of ramps) {
      if (Array.isArray(ramp?.points)) zoneMap.markHighway(ramp.points, { halfWidth: rampHalfWidth });
    }
  }
   ...
   */
  function sampleSurface(x, z, referenceY = Infinity) {
    const lateralTolerance = Math.max(0, readHighwayConfigNumber('surfaceQueryLateralTolerance', 0.15));
    ...
    for (const seg of surfaceSegments) {
      ...
      if (!best || height > best.height + 0.1 || (Math.abs(height - best.height) <= 0.1 && distSq < bestDistSq)) {
        best = {
          height,
          y: height,
          normal: seg.normal,
