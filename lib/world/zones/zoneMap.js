...
function createShorelineBuffer({ seed, width, length, cols }) {
  ...
  const maxInland = Math.max(
    minInland + 1,
    Math.min(length * 0.42, length - finiteOr(TERRAIN.beachWidth, 95) * 2),
  );
  ...
  for (let col = 0; col < cols; col++) {
    ...
    const broadBay =
      Math.sin(nx * Math.PI * 2.0 + phase) * 0.55 +
      Math.sin(nx * Math.PI * 4.7 + phase * 0.37) * 0.3 +
...
export function buildZoneMapFromSnapshot(snapshot) {
  ...
  if (
    ...
    !heightBuf ||
    !shorelineBuf ||
    zoneBuf.length !== expectedLength ||
    heightBuf.length !== expectedLength ||
    shorelineBuf.length !== cols
  ) {
    return null;
  }
