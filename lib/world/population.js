...
function resetHandleState(handle) {
  ...
  handle._fleeReplanDelay = AI.fleeReplanMin + Math.random() * Math.max(0, AI.fleeReplanMax - AI.fleeReplanMin);
  handle.avatarMat.rotation = 0;
  delete handle._fling;
  delete handle._onFlingComplete;
...
export function createPopulation({ scene, db, navGrid, buildingAABBs, onSplatMoot }) {
  ...
  function getHandles(reuseBuffer = false) {
    ...
    if (bossHandle && bossHandle.alive) liveHandlesBuffer.push(bossHandle);
