...
import { POP, BOSS, MOOT } from '../config.js';
...
export function createPopulation({ scene, db, navGrid, buildingAABBs, onSplatMoot }) {
  ...
  async function loadTex(row) {
    ...
    try {
      ...
      // Animated load failed — fall back to static chibi sprite, not the
      // video filename which would also fail.
  ...
  function getActiveRegulars() { return [...activeRegulars]; }
