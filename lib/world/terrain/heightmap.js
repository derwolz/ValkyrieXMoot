...
function drawWorldSegment(ctx, canvasSize, width, length, a, b, lineWidth, color) {
  if (!a || !b || !Number.isFinite(a.x) || !Number.isFinite(a.z) || !Number.isFinite(b.x) || !Number.isFinite(b.z) || lineWidth <= 0) return;
  ...
  if (bounds) {
    ...
    if (roadSegments.length > 0) {
      ...
      for (const xL of cityLayout?.xLines ?? []) {
        ...
        drawWorldRect(ctx, size, width, length, xL.x, (bounds.minZ + bounds.maxZ) / 2, xL.width, bounds.maxZ - bounds.minZ, roadColor);
      }
      for (const zL of cityLayout?.zLines ?? []) {
        const a = { x: bounds.minX, z: zL.z };
        const b = { x: bounds.maxX, z: zL.z };
        if (segmentTouchesWater(zoneMap, a, b)) continue;
        drawWorldRect(ctx, size, width, length, (bounds.minX + bounds.maxX) / 2, zL.z, bounds.maxX - bounds.minX, zL.width, roadColor);
      }
    ...
    for (const sw of cityLayout?.sidewalkPolys ?? []) {
      ...
      if (Array.isArray(sw.points) && sw.points.length >= 3) {
        ...
        drawWorldSegment(ctx, size, width, length, sw.a, sw.b, sw.width ?? CITY.sidewalkWidth, color);
      } else {
