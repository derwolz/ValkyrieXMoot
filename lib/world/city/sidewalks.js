...
function forSegments(start, end, maxLength, visit) {
  ...
  while ((direction > 0 && cursor < end) || (direction < 0 && cursor > end)) {
    const next =
      direction > 0 ? Math.min(cursor + maxLength, end) : Math.max(cursor - maxLength, end);
...
export function buildSidewalkLayout(
  bounds,
  xLines,
  zLines,
  alleys,
    ...
    for (let dist = 6; dist < len - 6; dist += step) {
      ...
      if (zone !== 'beach' && zone !== 'highway' && zone !== 'water') {
        interestPoints.push({
          x,
          z,
          kind: road.kind === 'feeder' ? 'ramp-feeder' : 'organic-road',
        });
      }
    }
  }
