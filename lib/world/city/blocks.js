...
export function insertAlleys(blocks, rand) {
  ...
  for (const b of blocks) {
    ...
    const longEnough =
      Math.max(w, d) >= CITY.alleyMinBlockDim && Math.min(w, d) >= CITY.alleyMinBlockDim * 0.6;
...
export function filterPlotsByZoneMask(plots, zoneMap, sampleStep = 18) {
  ...
  return plots.filter(
    (plot) => isUsablePlot(plot, 5) && !rectTouchesReservedZone(plot, zoneMap, sampleStep),
  );
}

export function filterPlotsNearRoads(plots, roadSegments, clearance = 0) {
  if (!Array.isArray(plots) || !Array.isArray(roadSegments) || roadSegments.length === 0)
    return plots;
  const minPlotDim = 5;
