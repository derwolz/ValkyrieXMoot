...
export function generateRoadGrid(rand, zoneMap = null) {
  ...
  const stepRand = (cx, axis) => {
    ...
    return baseSpacing * zoneSpacingMult * (1 - jitter + rand() * 2 * jitter);
  };
...
function promoteTiers(lines, rand) {
  const candidates = lines.map((l, i) => ({ l, i })).filter(({ l }) => !l.locked);
 ...
 */
export function generateHierarchicalRoadSegments(
  rand,
  zoneMap = null,
  highwayLayout = null,
  backboneSegments = [],
  ...
  const minIntersectionAngle =
    (Math.max(12, Number(cfg.minIntersectionAngleDeg) || 32) * Math.PI) / 180;
  ...
  const addPlannedSegment = (rawA, rawB, kind, width, source, hierarchy) => {
    ...
    if (
      !hasControlledIntersections(
        a,
        b,
        segments,
        nodes,
        minIntersectionSpacing,
  ...
  for (const connector of buildRadialConnectorPairs(
    waterfrontPoints,
    circuitPoints,
    radialConnectorCount,
  )) {
    addPlannedSegment(
      connector.a,
      connector.b,
...
export function generateVoronoiRoadSegments(
  rand,
  zoneMap = null,
  highwayLayout = null,
  backboneSegments = [],
...
export function applyRoundaboutsToRoadSegments(roadSegments, cfg = CITY.voronoiRoads ?? {}) {
  if (!Array.isArray(roadSegments) || roadSegments.length === 0) {
    return { roadSegments: [], roundabouts: [] };
  }

  const valenceThreshold = Math.max(3, Math.floor(Number(cfg.roundaboutValence) || 0));
  if (!valenceThreshold) return { roadSegments, roundabouts: [] };

  const radius = Math.max(6, Number(cfg.roundaboutRadius) || 18);
  const mergeRadius = Math.max(2, Number(cfg.roundaboutMergeRadius) || radius * 0.75);
  const roadOverlap = Math.max(0, Math.min(radius * 0.6, Number(cfg.roundaboutRoadOverlap) || 0));
  const loopCount = Math.max(8, Math.floor(Number(cfg.roundaboutSegments) || 14));
  const maxChordLength = Math.max(1.5, Number(cfg.roundaboutMaxChordLength) || 4);
  const loopWidth = Math.max(4, Number(cfg.roundaboutWidth) || 9);
  const baseSegments = roadSegments.filter((segment) => segment?.kind !== 'alley');
  const candidates = collectRoundaboutCandidates(baseSegments, mergeRadius);
  const roundaboutNodes = candidates.filter(
    (node) => uniqueDirectionCount(node.directions) >= valenceThreshold,
  );
  if (roundaboutNodes.length === 0) return { roadSegments, roundabouts: [] };
  ...
  for (const segment of roadSegments) {
    ...
    out.push(...cutSegmentAroundRoundabouts(segment, roundaboutNodes, radius, roadOverlap));
  }

  for (const node of roundaboutNodes) {
    const width = Math.max(loopWidth, medianSegmentWidth(node.incidents) * 0.55);
    const loop = buildRoundaboutLoopSegments(
      node.point,
      radius,
      loopCount,
      maxChordLength,
    ...
    roundabouts.push({
      ...
      radius,
      width,
      valence: uniqueDirectionCount(node.directions),
      segments: loop,
    });
  }

  return { roadSegments: out, roundabouts };
}

function collectRoundaboutCandidates(segments, mergeRadius) {
  const candidates = [];
  for (const segment of segments) {
    if (!isPointLike(segment?.a) || !isPointLike(segment?.b)) continue;
    addRoundaboutIncident(candidates, segment.a, makeSegmentIncident(segment, 0), mergeRadius);
    addRoundaboutIncident(candidates, segment.b, makeSegmentIncident(segment, 1), mergeRadius);
  }

  for (let i = 0; i < segments.length; i++) {
    const a = segments[i];
    if (!isPointLike(a?.a) || !isPointLike(a?.b)) continue;
    for (let j = i + 1; j < segments.length; j++) {
      const b = segments[j];
      if (!isPointLike(b?.a) || !isPointLike(b?.b)) continue;
      const hit = segmentIntersectionParams(a.a, a.b, b.a, b.b);
      if (!hit) continue;
      const point = {
        x: a.a.x + (a.b.x - a.a.x) * hit.t,
        z: a.a.z + (a.b.z - a.a.z) * hit.t,
      };
      addRoundaboutIncident(candidates, point, makeSegmentIncident(a, hit.t), mergeRadius);
      addRoundaboutIncident(candidates, point, makeSegmentIncident(b, hit.u), mergeRadius);
    }
  }
  return candidates;
}
...
function addRoundaboutIncident(candidates, point, incident, mergeRadius) {
  let candidate = null;
  let bestD2 = mergeRadius * mergeRadius;
  for (const existing of candidates) {
    const d2 = distanceSq(point, existing.point);
    if (d2 <= bestD2) {
      candidate = existing;
      bestD2 = d2;
    }
  ...
  if (!candidate) {
    candidate = { point: { x: point.x, z: point.z }, incidents: [], directions: [] };
    candidates.push(candidate);
  } else {
    const n = candidate.incidents.length;
    candidate.point.x = (candidate.point.x * n + point.x) / (n + 1);
    candidate.point.z = (candidate.point.z * n + point.z) / (n + 1);
  }

  candidate.incidents.push(incident);
  candidate.directions.push(...incident.directions);
}

function makeSegmentIncident(segment, t) {
  const directions = [];
  const eps = 1e-4;
  const point = pointOnSegment(segment, t);
  if (t > eps) directions.push(Math.atan2(segment.a.z - point.z, segment.a.x - point.x));
  if (t < 1 - eps) directions.push(Math.atan2(segment.b.z - point.z, segment.b.x - point.x));
  return {
    segment,
    t,
    width: Number(segment.width) || CITY.tiers?.local || 10,
    directions,
  };
}
...
function uniqueDirectionCount(directions) {
  return uniqueDirections(directions).length;
}
...
function uniqueDirections(directions) {
  ...
  for (const angle of directions) {
    ...
    if (!out.some((existing) => angularDistance(existing, angle) < minAngle)) out.push(angle);
  }
  return out;
}
...
function cutSegmentAroundRoundabouts(segment, roundaboutNodes, radius, roadOverlap = 0) {
  ...
  const cuts = [];
  for (const node of roundaboutNodes) {
    const projection = projectPointToSegment(node.point, segment.a, segment.b);
    if (projection.distance > radius * 1.35) continue;
    const t = distanceAlongSegment(segment.a, segment.b, projection.point) / len;
    const cutRadius = Math.max(radius * 0.25, radius - roadOverlap);
    const dt = Math.min(0.49, cutRadius / len);
    cuts.push({ from: clamp01(t - dt), to: clamp01(t + dt) });
  }
  ...
  for (const cut of cuts) {
    ...
    if (prev && cut.from <= prev.to) prev.to = Math.max(prev.to, cut.to);
  ...
  for (const cut of merged) {
    appendSegmentSlice(out, segment, cursor, cut.from, radius);
    cursor = Math.max(cursor, cut.to);
  }
  appendSegmentSlice(out, segment, cursor, 1, radius);
  return out;
}

function distanceAlongSegment(a, b, point) {
  const len = Math.hypot(b.x - a.x, b.z - a.z);
  if (len < 1e-6) return 0;
  return ((point.x - a.x) * (b.x - a.x) + (point.z - a.z) * (b.z - a.z)) / len;
}

function appendSegmentSlice(out, segment, from, to, radius) {
  if (to - from < Math.max(0.01, (radius * 0.25) / Math.max(1, segmentLength(segment)))) return;
  out.push({
    ...segment,
    a: pointOnSegment(segment, from),
    b: pointOnSegment(segment, to),
  });
}

function segmentLength(segment) {
  if (!isPointLike(segment?.a) || !isPointLike(segment?.b)) return 0;
  return Math.hypot(segment.b.x - segment.a.x, segment.b.z - segment.a.z);
}

function medianSegmentWidth(incidents) {
  const widths = incidents
    .map((incident) => Number(incident.width) || 0)
    .filter((width) => width > 0)
    .sort((a, b) => a - b);
  if (widths.length === 0) return CITY.tiers?.local || 10;
  return widths[Math.floor(widths.length / 2)];
}

function buildRoundaboutLoopSegments(
  center,
  radius,
  count,
  maxChordLength,
  ...
  for (let i = 0; i < count; i++) {
    ...
    byKey.set(pointKey(point), { ...point, angle });
  }
  for (const angle of uniqueDirections(directions)) {
    const point = pointOnCircle(center, radius, angle);
    byKey.set(pointKey(point), { ...point, angle });
  }

  const points = [...byKey.values()].sort((a, b) => a.angle - b.angle);

  const segments = [];
  for (let i = 0; i < points.length; i++) {
    const a = points[i];
    const b = points[(i + 1) % points.length];
    const arc = buildRoundaboutArc(center, radius, a.angle, b.angle, maxChordLength);
    segments.push({
      a,
      b,
      width,
      ...
      oneWay: true,
      arc,
      source: 'planned-roundabout-loop',
      hierarchy: 'roundabout',
    });
  }
  return segments;
}
...
function buildRoundaboutArc(center, radius, startAngle, endAngle, maxChordLength) {
  let delta = endAngle - startAngle;
  if (delta <= 0) delta += Math.PI * 2;
  const arcLength = radius * delta;
  const steps = Math.max(2, Math.ceil(arcLength / maxChordLength));
  const points = [];
...
function buildWaterfrontCollectorPoints(zoneMap, cfg, snapGrid, halfW, halfL) {
  const samples =
    typeof zoneMap?.getShorelineSamples === 'function' ? zoneMap.getShorelineSamples() : [];
  ...
  if (isPointLike(last))
    points.push(snapPoint({ x: last.x, z: last.z + offset }, snapGrid, halfW, halfL));
...
function buildCircuitCollectorPoints(highwayLayout, cfg, snapGrid, halfW, halfL) {
  ...
  const out = [];
  let distanceSincePick = Number.POSITIVE_INFINITY;
  for (let i = 0; i < points.length; i++) {
    const p = points[i];
    if (!isPointLike(p)) continue;
    if (i > 0 && isPointLike(points[i - 1]))
      distanceSincePick += Math.hypot(p.x - points[i - 1].x, p.z - points[i - 1].z);
    if (distanceSincePick < spacing && i !== points.length - 1) continue;
    distanceSincePick = 0;
    const inward = normalizeVector(-p.x, -p.z);
    out.push(
      snapPoint({ x: p.x + inward.x * offset, z: p.z + inward.z * offset }, snapGrid, halfW, halfL),
    );
  }
...
function buildRadialConnectorPairs(waterfrontPoints, circuitPoints, count) {
  ...
  const pairs = [];
  const sortedCircuit = [...circuitPoints].sort(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const a =
      sortedCircuit[
        Math.min(sortedCircuit.length - 1, Math.max(0, Math.round(t * (sortedCircuit.length - 1))))
      ];
    if (!a) continue;
  ...
  if (waterfrontPoints.length >= 2) {
    ...
    for (let i = 1; i < count; i += 3) {
      ...
      const waterIndex = Math.min(
        sortedWaterfront.length - 1,
        Math.max(0, Math.round(t * (sortedWaterfront.length - 1))),
      );
...
function getDistrictLocalDensity(zone, cfg) {
  ...
  if (zone === 'rural') return clamp01(Number(cfg.ruralLocalDensity) || 0.3);
  return 0;
}
...
function getDistrictLocalDirections(zone, rand) {
  ...
  const pool =
    zone === 'urban'
      ? [...cardinal, ...diagonal]
      : zone === 'suburban'
        ? [...cardinal, Math.PI / 4, (-3 * Math.PI) / 4]
        : cardinal;
...
function hasControlledIntersections(a, b, segments, nodes, minSpacing, minAngle) {
  for (const segment of segments) {
    ...
    if (distanceToSegmentEndpoint(intersection, segment.a, segment.b) < minSpacing * 0.5)
      return false;
  }
  return true;
}
...
export function splitRoadSegmentsByReservedZones(roadSegments, zoneMap, reservationSegments = []) {
  ...
  if (!zoneMap && (!Array.isArray(reservationSegments) || reservationSegments.length === 0))
    return roadSegments;
  const out = [];
  for (const segment of roadSegments) {
...
export function filterIntersectionsByReservedZones(
  intersections,
  zoneMap,
  reservationSegments = [],
) {
  ...
  return intersections.filter((intersection) => {
    ...
    const halfWidth =
      Math.max(Number(intersection?.widthX) || 0, Number(intersection?.widthZ) || 0) * 0.5;
    return (
      isPointLike(pos) && !isReservedRoadZone(pos.x, pos.z, zoneMap, reservationSegments, halfWidth)
    );
  });
}
...
function splitRoadSegmentByReservedZones(segment, zoneMap, reservationSegments = []) {
  ...
  const sampleStep = Math.max(
    5,
    Math.min(12, (Number(segment.width) || CITY.tiers.local || 10) * 0.75),
  );
...
function projectPointToSegment(point, a, b) {
  ...
  const t = lenSq > 1e-6 ? clamp(((point.x - a.x) * abx + (point.z - a.z) * abz) / lenSq, 0, 1) : 0;
  const px = a.x + abx * t;
  const pz = a.z + abz * t;
  return {
    point: { x: px, z: pz },
...
function isReservedRoadZone(x, z, zoneMap, reservationSegments = [], margin = 0) {
  if (
    Array.isArray(reservationSegments) &&
    reservationSegments.length > 0 &&
    pointTouchesReservation(x, z, reservationSegments, margin)
  )
    return true;
...
function pointTouchesReservation(x, z, reservationSegments, margin = 0) {
  ...
  for (const reservation of reservationSegments) {
    ...
    const halfWidth = Math.max(
      0,
      Number(reservation.halfWidth) || (Number(reservation.width) || 0) * 0.5,
    );
    if (
      projectPointToSegment(point, reservation.a, reservation.b).distance <=
      halfWidth + Math.max(0, margin)
    )
      return true;
  }
  return false;
