...
import { generateRoadGrid, gridToSegments, generateRampFeederSegments, generateHierarchicalRoadSegments, splitRoadSegmentsByReservedZones, filterIntersectionsByReservedZones } from './roads.js';
import { extractBlocks, insertAlleys, subdividePlots, filterPlotsNearRoads, filterPlotsByZoneMask } from './blocks.js';
 ...
 */
export function generateCityLayout({ seed = CITY.seed, zoneMap = null, highwayLayout = null } = {}) {
  const rand = mulberry32(seed);

  const { xLines, zLines } = generateRoadGrid(rand, zoneMap);
  const { segments: rawGridSegments, intersections: rawIntersections } = gridToSegments(xLines, zLines);
  const highwayReservationSegments = highwayReservationsToSegments(highwayLayout?.reservationPolylines);
  const segments = splitRoadSegmentsByReservedZones(rawGridSegments, zoneMap, highwayReservationSegments);
  const intersections = filterIntersectionsByReservedZones(rawIntersections, zoneMap, highwayReservationSegments);
  const rawPlannedRoadSegments = generateHierarchicalRoadSegments(rand, zoneMap, highwayLayout, segments);
  const plannedRoadSegments = splitRoadSegmentsByReservedZones(rawPlannedRoadSegments, zoneMap, highwayReservationSegments);
  const rampFeederSegments = generateRampFeederSegments(highwayLayout, [...segments, ...plannedRoadSegments], zoneMap);
  const organicRoadSegments = [...plannedRoadSegments, ...rampFeederSegments];
  ...
  const roadSegments = [...segments, ...alleys, ...organicRoadSegments];
  const sidewalkLayout = buildSidewalkLayout(bounds, xLines, zLines, alleys, zoneMap, organicRoadSegments);
 ...
 * }} opts
 */
export function generateCity({ scene, seed = CITY.seed, zoneMap = null, highwayLayout = null, getTerrainY = null, layout = null }) {
  ...
  const interestPoints = buildCityGround(scene, bounds, xLines, zLines, alleys, zoneMap, getTerrainY, cityLayout);
  const { aabbs: plotBuildingAABBs } = placeBuildings(scene, buildablePlots, rand, getTerrainY);
  ...
  // collision.
  const buildingAABBs = [...plotBuildingAABBs, ...boundaryAABBs];
...
export function hydrateCityLayout(snapshot, seed = CITY.seed) {
  ...
  const buildingSeed = Number.isFinite(Number(snapshot.buildingSeed)) ? Number(snapshot.buildingSeed) : layoutSeed;
  return {
    ...snapshot,
    seed: layoutSeed,
    buildingSeed,
