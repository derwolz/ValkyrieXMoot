...
function addBoundaryRow({ edge, bounds, ridgeCfg, buildingCfg, getTerrainY, zoneMap, ridgeInstances, buildingInstances, roofInstances, aabbs }) {
  ...
  for (let i = 0; i < ridgeCount; i++) {
    ...
    const cx = alongZ
      ? (negativeSide ? bounds.minX - ridgeDepth / 2 : bounds.maxX + ridgeDepth / 2)
      : alongCenter;
    const cz = alongZ
      ? alongCenter
      : (negativeSide ? bounds.minZ - ridgeDepth / 2 : bounds.maxZ + ridgeDepth / 2);
  ...
  for (let i = 0; i < buildingCount; i++) {
    ...
    const cx = alongZ
      ? (negativeSide ? bounds.minX - buildingDepth / 2 : bounds.maxX + buildingDepth / 2)
      : alongCenter;
    const cz = alongZ
      ? alongCenter
      : (negativeSide ? bounds.minZ - buildingDepth / 2 : bounds.maxZ + buildingDepth / 2);
...
export function placeBoundaryBlockers(scene, bounds, getTerrainY = null, zoneMap = null) {
  ...
  addInstancedBoundaryMesh(
    scene,
    _unitGeo,
    createFacadeMaterial({ textureIndex: BOUNDARY_BUILDING_TEXTURE_INDEX, name: 'building-boundary-oversized-sides' }),
    buildingInstances,
