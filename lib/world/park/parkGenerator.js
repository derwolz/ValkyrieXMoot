...
const _trunkGeo = new THREE.CylinderGeometry(
  PARK.trunkRadius,
  PARK.trunkRadius * 1.1,
  PARK.trunkHeight,
  6,
);
...
export function buildParkAssets({ scene, zoneMap, getTerrainY, seed = CITY.seed }) {
  ...
  function terrainYAt(x, z) {
    const y =
      typeof getTerrainY === 'function'
        ? getTerrainY(x, z)
        : typeof zoneMap.getHeight === 'function'
          ? zoneMap.getHeight(x, z)
          : 0;
  ...
  function emitPath(fromX, toX, wz) {
    ...
    const segmentLenMax = Math.max(
      PARK.pathWidth,
      PARK.pathSegmentLength || CITY.infrastructureSegmentLength || totalLen,
    );
  ...
  function dispose() {
    for (const m of meshes) {
      ...
      if (
        m.geometry &&
        m.geometry !== _trunkGeo &&
        m.geometry !== _canopyGeo &&
        m.geometry !== _benchGeo
      ) {
        m.geometry.dispose();
      }
      if (m.material) {
