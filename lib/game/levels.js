...
export function pickBossSpawn(playerSpawn, bounds, navGrid) {
  ...
  return (
    snapToWalkable({ x: rawX, z: rawZ }, navGrid, LEVEL.bossSnapRadius) || { x: rawX, z: rawZ }
  );
}
...
export function snapToWalkable(pos, navGrid, maxRadius) {
  const { worldToCell, cellToWorld, idx, walkable, cellSize } = navGrid;
