...
export const SORTERS = {
  ...
  username: (a, b) => (a.username || '').toLowerCase().localeCompare((b.username || '').toLowerCase()),
  display_name: (a, b) => (a.display_name || '').toLowerCase().localeCompare((b.display_name || '').toLowerCase()),
  match_distance: (a, b) => (a.match_distance ?? Infinity) - (b.match_distance ?? Infinity),
  has_chibi: (a, b) => {
...
export function applySort(key, moots, handles) {
  ...
  for (let pos = 0; pos < order.length; pos++) {
    ...
    h.group.position.set(
      (hashRandom(pos) * 2 - 1) * MOOT.jitterX,
      0,
      -pos * MOOT.spacing,
    );
  }
