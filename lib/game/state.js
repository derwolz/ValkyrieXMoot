...
export function createGameState(deps) {
  const { vehicle, camera, getMootHandles, impactSystem, pistol, updateHud } = deps;
  ...
  function splatMoot(handle, weapon, launch, legacyDirZ) {
  ...
  function resetRunState() {
    ...
    game.combo = 1;
    if (game.targetHandle && deps.releaseObjectiveTarget)
      deps.releaseObjectiveTarget(game.targetHandle);
  ...
  function ramMoots(handles) {
    ...
    vehicle.speed *= 0.95;
    if (objectiveRammed)
      setFace(game.boosting ? 'turboCelebrating' : 'celebrating', MIRROR.celebratingMs);
  ...
  function tryShoot(aim) {
    ...
    let o = aim.object;
    let hitMoot = false;
    let objectiveShot = false;
    while (o) {
      if (o.userData?.mootHandle) {
        ...
        const sdx = aim.point.x - vehicle.position.x;
        const sdz = aim.point.z - vehicle.position.z;
      ...
      if (o.userData?.isNpcVehicle) {
        const npcHandle = o.userData.npcHandle;
        if (npcHandle?.alive && deps.onNpcVehicleShot) {
