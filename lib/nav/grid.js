...
function pointSegmentDistanceSq(wx, wz, segment) {
  ...
  const t = Math.max(0, Math.min(1, ((wx - segment.ax) * segment.dx + (wz - segment.az) * segment.dz) / segment.lenSq));
...
function isWaterCell(zoneMap, wx, wz, zone = null) {
  ...
  if (typeof zoneMap.getWaterInfo === 'function' && zoneMap.getWaterInfo(wx, wz)?.isWater) return true;
