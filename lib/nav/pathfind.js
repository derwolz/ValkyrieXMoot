  ...
  while (s.heapSize > 0 && iter++ < maxIter) {
    ...
    for (let dr = -1; dr <= 1; dr++) {
      ...
      for (let dc = -1; dc <= 1; dc++) {
        ...
        const oldG = cellGen[ni] === gen ? g[ni] : Number.POSITIVE_INFINITY;
...
function reconstructPath(parent, goalIdx, cols, cellToWorld) {
  ...
  while (cur !== -1) {
    ...
    path[write--] = cellToWorld(col, row);
    cur = parent[cur];
  }
 ...
 * Compacts the path array in place to avoid a second output array.
 */
function smoothPath(path) {
  const n = path.length;
  if (n <= 2) return path;
  let write = 1;
  for (let read = 1; read < n - 1; read++) {
    const prev = path[write - 1];
    const cur = path[read];
    const next = path[read + 1];
