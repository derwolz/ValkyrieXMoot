...
export function setupDebugPanel({ pistol }) {
  ...
  p.add(PISTOL, 'anchorX', -1.5, 1.5, 0.01).onChange((v) => {
    pistol.group.position.x = v;
  });
  p.add(PISTOL, 'anchorY', -1.0, 1.0, 0.01).onChange((v) => {
    pistol.group.position.y = v;
  });
  p.add(PISTOL, 'anchorZ', -3, -0.2, 0.01).onChange((v) => {
    pistol.group.position.z = v;
  });
  ...
  g.add(GAME, 'ammoPerRam', 0, 50, 1);
  g.add(GAME, 'firingCostAmmo', 0, 100, 1);
  g.close();
