...
function findBodyOverlap(playerBody, npcBody) {
  ...
  for (const rawAxis of axes) {
    ...
    const overlap = Math.min(playerProjection.max, npcProjection.max) - Math.max(playerProjection.min, npcProjection.min);
...
function getPlayerFrontContactPoint(playerBody, vehicle) {
  const forwardSpeed = finiteNumber(vehicle?.speed, playerBody.velocity.x * playerBody.forward.x + playerBody.velocity.z * playerBody.forward.z);
  ...
  const lateralDenominator = Math.max(Math.abs(forwardSpeed), Math.abs(lateralSpeed), CAR.impactMinRelativeSpeed);
  const lateralRatio = clamp(lateralSpeed / lateralDenominator, -1, 1);

  return {
    x: playerBody.center.x + playerBody.forward.x * playerBody.halfExtents.z + playerBody.right.x * playerBody.halfExtents.x * lateralRatio,
    z: playerBody.center.z + playerBody.forward.z * playerBody.halfExtents.z + playerBody.right.z * playerBody.halfExtents.x * lateralRatio,
  };
}
...
function getYawInertia(body) {
  ...
  return Math.max(BODY_AXIS_EPSILON, body.mass * (width * width + length * length) / BODY_INERTIA_DIVISOR);
}

/**
 * Map-edge collision is intentionally disabled.
 *
 * The world must not stop the player with an invisible boundary; vehicle-stopping
 * collisions should only come from visible, player-facing objects. Keep this
 * exported no-op so older callers or debug scripts do not fail if they still
 * import it.
 *
 * @returns {null}
 */
export function resolveMapBounds(_vehicle, _bounds) {
  return null;
}
...
export function resolveNpcVehicleBodyCollision(vehicle, npcVehicle) {
  ...
  const velocityAlongNormal = relativeVelocity.x * overlap.normal.x + relativeVelocity.z * overlap.normal.z;
  ...
  const angularVelocityDeltaY = torqueImpulseY / getYawInertia(npcBody) * NPC_VEHICLE.impactSpinStrength;
  ...
  return {
    ...
    contactPoint: makeVector3Like(contactPoint.x, getVectorComponent(playerDescriptor.center, 'y'), contactPoint.z),
    impulseMagnitude,
    playerVelocityDelta,
    npcVelocityDelta,
