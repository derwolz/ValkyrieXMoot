...
export function tickPerception(dt, moot, ctx) {
  ...
  const { truckPos, truckVelX, truckVelZ, buildingAABBs, now, recentKills, recentShots } = ctx;
  ...
  if (distTruck < 8) {
    ...
    // hasLOS accepts any {x,z}-shaped object; pass an inline literal only here
    // (one allocation per LOS check, not every frame).
    if (hasLOS({ x: mpx, z: mpz }, truckPos, buildingAABBs)) {
      threatDelta += 0.6;
    }
  }
