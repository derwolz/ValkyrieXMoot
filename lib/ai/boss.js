...
export function tickBoss(dt, moot, ctx) {
  ...
  _tickPursue(dt, moot, pos, navGrid, buildingSource);
}

// ---------------------------------------------------------------------------
// Engage sub-tick: kite at BOSS.kiteDistance, fire at truck.
// ---------------------------------------------------------------------------
function _tickEngage(
  dt,
  moot,
  pos,
  truckPos,
  ...
  if (Math.abs(kiteErr) > 1.0 && dist > 0.01) {
    ...
    moveWithCollision(pos, vx, vz, 1, buildingAABBs, moot.collisionRadius, pos);
  }
...
function _tickPursue(dt, moot, pos, navGrid, buildingAABBs) {
  ...
  if (needsReplan) {
    moot.path = findPath(navGrid, pos, target);
    moot.pathIndex = 0;
